// English message catalog.
// Sibling shape with src/i18n/zh-TW.ts — adding/removing keys requires updating both.
// `messages` is intentionally NOT `as const` so zh-TW.ts can mirror the shape
// with different string values. The `as const` on `common.sources`/`common.decisions`
// is kept so the keys (ARXIV, RECOMMEND, ...) stay a typed record.

export const messages = {
  header: {
    brand: 'PaperScout',
    searchAria: 'Search papers',
    searchPlaceholder: 'Search by title, authors, keywords, or topic…',
    keyboardHint: '⌘ K',
    navHot: 'Hot',
    navLatest: 'Latest',
    navLibrary: 'My library',
    navAllPapers: 'All papers',
    navUpload: 'Upload paper',
    navUploadTitle: 'V1 ingests data via local skills only',
    notificationsAria: 'Notifications',
    avatarAria: 'User avatar',
    accountMenuAria: 'Open account menu',
    mainNavAria: 'Main menu',
    localeSwitcherAria: 'Language',
    localeSwitcherOptionEn: 'English',
    localeSwitcherOptionZhTw: '繁體中文',
    signInWithGoogle: 'Sign in with Google',
    signedInAs: (name: string) => `Signed in as ${name}`,
    userMenu: {
      account: 'Account',
      viewHistory: 'View history',
      viewHistoryDisabledTitle: 'View paper history',
      collect: 'Collect',
      collectDisabledTitle: 'Open my library',
      signOut: 'Sign out',
      signOutFailed: 'Sign out failed. Please try again.',
    },
  },
  home: {
    emptyTitle: 'No agent run imported yet',
    emptyBody:
      'PaperScout is read-only right now. Run the collection and evaluation skills locally, then ingest the run directory.',
    emptyCommands:
      '# 1. Collect candidates\n#    Claude Code -> /collect-papers\n# 2. Evaluate the top 15\n#    Claude Code -> /evaluate-papers\n# 3. Ingest into the DB\nnpm run ingest data/runs/<YYYY-MM-DD-HHMM>',
    emptyLibraryHint: 'You can still browse the',
    emptyLibraryLink: 'library',
    emptyLibraryAfter: 'for previously stored papers.',
    heroTitle: 'Find papers worth your time, faster.',
    heroSubtitle:
      'The AI summarizes, scores, and tags the most relevant papers from the latest agent run.',
    heroSearchAria: 'Enter a research interest',
    heroSearchPlaceholder: 'Type a topic you care about — e.g. LLM agents, RAG, diffusion models…',
    heroSubmitAria: 'Search papers',
    heroSubmitTitle: 'Search all papers',
    heroTopicsLabel: 'Hot topics:',
    feedTabRecommended: 'For you',
    feedTabTrending: 'Trending',
    feedTabLatest: 'Latest',
    feedTabTop: 'Top scored',
    feedLatestCountAria: (count: number) => `${count} papers in the latest update`,
    feedLatestUpdatedTooltip: (time: string) => `Updated: ${time}`,
    feedTablistAria: 'Paper feed filters',
    feedFilterDomain: 'Conference',
    feedFilterTime: 'Time',
    feedFilterSort: 'Sort: AI score',
    feedTimeWeek: 'Past week',
    feedTimeMonth: 'Past month',
    feedTimeYear: 'Past year',
    feedSortDate: 'Sort: Newest',
    feedSortRank: 'Sort: Rank',
    feedSortAria: 'Sort by',
    feedViewAria: 'View mode',
    feedViewCard: 'Card view',
    feedViewList: 'List view',
    feedControlsAria: 'List controls',
    feedTitleSr: 'Recommended papers',
    feedNoResults: 'No papers produced in this run yet.',
    feedViewAllPapers: 'View all papers →',
    feedComingSoon: 'Coming soon',
    feedComingSoonBody: 'Trending will rank papers by reader views — coming soon.',
    loadingHero: 'Loading trend summary',
    loadingFeedMeta: 'Loading feed status',
    loadingFeed: 'Loading paper feed',
    loadingHotTags: 'Loading hot tags',
    loadingRecommendations: 'Loading recent recommendations',
    loadingSourceMix: 'Loading source mix',
    summaryFallback:
      'This paper is ingested into PaperScout — open it to see sources, summary, and full evaluation.',
    reasonFallback: 'This paper was flagged as recommended in the agent evaluation.',
    cardFigureFallback: 'Highlight figure',
    cardViewSummary: 'View summary',
    cardSummaryDialogAiLabel: 'AI summary',
    cardSummaryDialogAbstractLabel: 'Abstract',
    cardSummaryDialogEmpty: 'No abstract is available for this paper yet.',
    cardSummaryDialogViewFull: 'View full details →',
    cardOpenPaper: 'Open paper ↗',
    cardFavorite: 'Favorite',
    cardReadLater: 'Read later',
    cardPublishDate: (date: string) => `Published: ${date}`,
    cardReasonHeader: 'Why recommended',
    runMeta: (date: string, total: number, recommended: number, page: number) =>
      `Latest agent update: ${date} · ${total} papers collected · ${recommended} recommended · showing page ${page}`,
    viewFullRun: 'View full run →',
    sidebarAria: 'Auxiliary information',
    hotTagsTitle: 'Hot tags',
    hotTagsLink: 'View all',
    hotTagsDialogTitle: 'All tags',
    hotTagsSearchPlaceholder: 'Search tags',
    hotTagsNoMatch: 'No tags match your search.',
    hotTagsEmpty: 'No tags yet.',
    recentTitle: 'Recent recommendations',
    recentLink: 'View all',
    sourceMixTitle: 'Source mix',
    sourceMixEmpty: 'No source data yet.',
    personalTitle: 'Personalize your feed',
    personalBody: 'Let the we learn your taste for sharper recommendations.',
    personalCta: 'Set my research interests',
    paginationAllShown: (n: number) => `All ${n} paper(s) shown`,
    paginationStatus: (current: number, total: number, items: number) =>
      `Page ${current} / ${total} · ${items} paper(s) total`,
    paginationPrev: 'Previous',
    paginationNext: 'Next',
    paginationAria: 'Paper pagination',
  },
  library: {
    title: 'My library',
    personalSubtitle: 'Personal lists, likes, notes, reading status, and paper history.',
    paperCount: (n: number) => `${n} paper(s)`,
    empty: 'No papers stored yet. Run',
    emptyOr: 'or ingest a run via',
    allPapers: 'All papers',
    liked: 'Liked',
    history: 'History',
    collections: 'Personal lists',
    newList: 'Lists',
    newListPlaceholder: 'New list name',
    createList: 'Create',
    manageList: 'Manage list',
    renameList: 'Rename',
    deleteList: 'Delete list',
    deleteListWarning: `Delete the list "{name}"? This can't be undone, but the papers will stay in your library.`,
    cancel: 'Cancel',
    addPapers: 'Add papers',
    addToList: 'Add',
    addToListHeading: 'Add to list',
    removeFromList: 'Remove from list',
    removeFromLibrary: 'Remove',
    openPaper: 'Open',
    notePlaceholder: 'Plain text notes...',
    saveNote: 'Note',
    emptyPersonalTitle: 'No papers here yet',
    emptyPersonalBody:
      'Add papers from the available papers section or open paper details to build history.',
    searchPlaceholder: 'Search your collection...',
    statusFilterAll: 'All statuses',
    statusFilterAria: 'Filter by reading status',
    tagFilterAll: 'All tags',
    tagFilterAria: 'Filter by tag',
    tagSearchPlaceholder: 'Search tags…',
    tagNoResults: 'No matching tags',
    sortAria: 'Sort papers',
    sortRecent: 'Recently opened',
    sortAdded: 'Recently added',
    sortScore: 'Highest score',
    reset: 'Reset',
    statusLabel: 'Status',
    likedLabel: 'Like',
    lastViewed: 'Viewed',
    paginationPrev: 'Previous',
    paginationNext: 'Next',
    paginationRange: 'Showing {from}–{to} of {total}',
    metricTotal: 'Total',
    metricLiked: 'Liked',
    metricUnread: 'Unread',
    metricReading: 'Reading',
    metricRead: 'Read',
    metricNotes: 'Notes',
    metricHistory: 'History',
    metricTotalHelp: 'Total papers collected',
    metricUnreadHelp: 'Not started reading yet',
    metricReadingHelp: 'Papers in progress',
    metricReadHelp: 'Finished reading',
    metricNotesHelp: 'Total notes across papers',
    libraryHeading: 'My library',
    readingStatusHeading: 'Reading status',
    aiSummary: 'AI summary',
    updateFailed: 'Update failed. Please retry.',
    personalSummaryFallback:
      'This paper is in your personal workspace. Add notes and status as you read.',
    signInTitle: 'Sign in to use your paper collection',
    signInBody: 'Lists, likes, notes, status, and reading history are stored per user.',
    signInCta: 'Sign in with Google',
    statuses: {
      UNREAD: 'Unread',
      READING: 'Reading',
      READ: 'Read',
      ARCHIVED: 'Archived',
    },
    columnTitle: 'Title',
    columnAuthors: 'Authors',
    columnSource: 'Source',
    columnPublished: 'Published',
    columnStored: 'Stored',
  },
  papers: {
    title: 'All papers',
    subtitle:
      'Browse every paper in the database. Search, filter, and sort across all collected work.',
    searchPlaceholder: 'Search title, abstract, or authors…',
    searchAria: 'Search all papers',
    searchSubmit: 'Search',
    conferenceAll: 'All conferences',
    conferenceAria: 'Filter by conference',
    tagAll: 'All tags',
    tagAria: 'Filter by tag',
    tagSearchPlaceholder: 'Search tags…',
    tagNoResults: 'No matching tags',
    sortAria: 'Sort papers',
    sortNewest: 'Newest',
    sortOldest: 'Oldest',
    sortScore: 'Recommended first',
    reset: 'Reset',
    rangeLabel: (from: number, to: number, total: number) =>
      `Showing ${from}–${to} of ${total} paper(s)`,
    emptyTitle: 'No papers yet',
    emptyBody: 'Once runs are ingested, every collected paper will appear here.',
    noResultsTitle: 'No matching papers',
    noResultsBody: 'Try a different search term, or clear the filters.',
    loading: 'Loading papers',
  },
  runPage: {
    eyebrow: (date: string) => `Agent run · ${date}`,
    heading: (date: string) => `Paper trends for ${date}`,
    status: (status: string) => `Status: ${status}`,
    completedAt: (date: string) => `Completed ${date}`,
    failedBody: 'This run failed before any papers were ranked.',
    runningBody: 'Collection/import in progress. Reload after the agent run completes.',
    sourceMixHeader: 'Source mix',
    sectionAll: 'All ranked papers',
    sectionRecommended: 'Recommended papers',
    showRecommendedOnly: 'Show recommended only',
    showAll: 'Show all results',
    emptyAll: 'This run produced no ranked papers.',
    emptyRecommended: 'No papers in this run were marked RECOMMEND.',
  },
  paperPage: {
    backToRun: 'Back to latest run',
    backToLibrary: 'Back to library',
  },
  paperCard: {
    stageAbstract: 'Abstract',
    stageFullPdf: 'Full PDF',
    figureFallback: 'Highlight figure',
  },
  paperDetail: {
    stageAbstract: 'Abstract screening',
    stageFullPdf: 'Full PDF analysis',
    publishedPrefix: 'Published',
    figureFallback: 'Highlight figure',
    figureCredit: (parts: {
      title: string;
      authors: string;
      venue: string | null;
      figureLabel: string | null;
    }) => {
      const segs = [`“${parts.title}”`, parts.authors];
      if (parts.venue) segs.push(parts.venue);
      if (parts.figureLabel) segs.push(parts.figureLabel);
      return `Image source: ${segs.join(', ')}. Copyright remains with the original author(s)/rights holder(s); reproduced here for academic introduction and commentary only.`;
    },
    figureCreditSourceLabel: 'Original paper',
    summary: 'Summary',
    whyRecommended: 'Why recommended',
    strengths: 'Strengths',
    weaknesses: 'Weaknesses',
    links: 'Links',
    actionsHeader: 'Your activity',
    commentsHeader: 'Comments',
    commentsEmpty: 'No comments yet. Be the first to share your thoughts.',
    commentPlaceholder: 'Share your thoughts on this paper…',
    commentPost: 'Post',
    commentPosting: 'Posting…',
    commentFailed: 'Failed to post. Retry.',
    commentDelete: 'Delete comment',
    commentAnonymous: 'Anonymous',
    commentSignInCta: 'Sign in',
    commentSignInBody: 'to join the discussion.',
    save: 'Save',
    saved: 'Saved',
    statusHeader: 'Reading status',
    notesHeader: 'Notes',
    noteSavedJustNow: 'Saved · just now',
    noteSaving: 'Saving…',
    noteSaveFailed: 'Save failed. Retry.',
    signedOutTitle: 'Sign in to organize',
    signedOutBody: 'Save papers, set reading status, and keep notes once you sign in.',
    signedOutCta: 'Sign in with Google',
    aiDigest: 'AI Digest',
    digestTldr: 'Read it in one sentence.',
    digestProblemMotivation: 'Problem & Motivation',
    digestKeyContributions: 'Key Contributions',
    digestMethodOverview: 'Method Overview',
    digestResultsInterpretation: 'Results Interpretation',
    digestAiCommentary: 'AI Commentary',
  },
  sourceMix: {
    empty: 'No sources yet.',
  },
  trendSummary: {
    papers: 'Papers',
    recommended: 'Recommended',
    pdfAnalysis: 'PDF analysis',
    topSource: 'Top source',
    pdfHint: (ok: number, unavail: number, none: number) =>
      `${ok} ok · ${unavail} unavail · ${none} abstract`,
  },
  trendTags: {
    empty: 'No tags yet.',
    more: (n: number) => `+${n} more`,
  },
  common: {
    dash: '—',
    pdf: 'PDF ↗',
    code: 'Code ↗',
    sources: {
      ARXIV: 'arXiv',
      OPENREVIEW: 'OpenReview',
      HUGGINGFACE: 'Hugging Face',
      OPENACCESS: 'Open Access',
    } as const,
    decisions: {
      RECOMMEND: 'RECOMMEND',
      STORE_ONLY: 'STORE_ONLY',
      LOW_QUALITY: 'LOW_QUALITY',
    } as const,
  },
  metadata: {
    title: 'PaperScout',
    description: 'Read-only viewer for agent-collected paper trends.',
  },
  account: {
    title: 'Account',
    emailLabel: 'Email',
    nameLabel: 'Display name',
    namePlaceholder: 'Your name',
    localeLabel: 'Preferred language',
    localeHelp: 'Used the next time you sign in.',
    lastLoginLabel: 'Last sign-in',
    memberSinceLabel: 'Member since',
    save: 'Save changes',
    saving: 'Saving…',
    savedAt: (when: string) => `Saved at ${when}`,
    saveFailed: 'Could not save changes. Please try again.',
    nameRequired: 'Name cannot be empty.',
  },
  footer: {
    ariaLabel: 'Site footer',
    tagline:
      'Find papers worth your time, faster. The AI summarizes, scores, and tags the most relevant work for you.',
    resourcesHeading: 'Resources',
    navAbout: 'About us',
    navFaq: 'FAQ',
    navHowItWorks: 'How it works',
    contactHeading: 'Contact',
    email: 'Email us',
    emailHint: 'Questions or feedback?',
    topicsHeading: 'Hot topics',
    copyright: '© 2026 PaperScout. All rights reserved. AI summaries and recommendations are for research assistance only and may contain errors or omissions. Please refer to the original papers and official publication information.',
    privacy: 'Privacy Policy',
    terms: 'Terms of Service',
  },
  placeholder: {
    badge: 'Coming soon',
    body: 'This page is still being built. Check back soon.',
    backHome: 'Back to home',
    titles: {
      about: 'About us',
      faq: 'FAQ',
      howItWorks: 'How recommendations work',
      privacy: 'Privacy Policy',
      terms: 'Terms of Service',
    },
  },
};

export type Messages = typeof messages;
